...
`ssh` は組み込みではなく、1 つのリモートコマンドを実行するローカルプログラムを指定する `remote.command` の既定値です。
...
Herdr はこの引数ベクタで `program` を直接起動します。ローカルシェルは介在しないため、`{target}`（`--remote` に渡した値）と `{command}`（Herdr が組み立てたリモートコマンド）は内容にかかわらずそれぞれ正確に 1 個の引数のままです。`{ssh_options}` は単独の要素として書く必要があり、その呼び出しで Herdr が管理する OpenSSH オプション（上記の生成された設定とコントロールソケット、保存済みマシンに必要な非対話オプション、stdin を読まないコマンド向けの `-n`）に展開されます。これを必要とするのは ssh のコマンドラインだけです。

サンドボックス、コンテナ、クラスタの CLI など別の方法でターゲットに接続するには、両方を置き換えます。
...
プログラムは `ssh target "<command>"` と同じようにそのコマンドをターゲット上で実行し、stdin と stdout を接続したままにする必要があります。長時間動作するクライアントブリッジとリモートインストールがそれらを使います。同じランチャーは保存済み SSH マシンを含むすべてのリモートコマンドで使われます。`{ssh_options}` がなければ Herdr は SSH 設定を生成しないため、`manage_ssh_config` の適用対象もありません。Herdr が使用できないランチャー設定はエラーであり、黙ってフォールバックすることはありません。
