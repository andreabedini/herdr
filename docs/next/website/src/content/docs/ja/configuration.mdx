...
`ssh` 以外の方法でターゲットに接続するには、Herdr がローカルで実行するプログラムを設定します。
...
Herdr はこの引数ベクタで `program` を直接起動します。ローカルシェルは介在しないため、`{target}` と `{command}` は内容にかかわらずそれぞれ正確に 1 個の引数のままです。`{target}` は `--remote` に渡した値、`{command}` は Herdr が組み立てたリモートコマンドです。プログラムは `ssh target "<command>"` と同じようにそのコマンドをターゲット上で実行し、stdin と stdout を接続したままにする必要があります。長時間動作するクライアントブリッジとリモートインストールがそれらを使います。同じランチャーは保存済み SSH マシンを含むすべてのリモートコマンドで使われ、Herdr は SSH 設定を生成しないため `manage_ssh_config` は適用されません。使用できないランチャー設定はエラーになり、`ssh` へ黙ってフォールバックすることはありません。
