...
`ssh` is only the default launcher. Set `[remote.command]` to run remote commands through another program, for example a sandbox or container CLI; see [Configuration](/docs/configuration/#remote-attach).
