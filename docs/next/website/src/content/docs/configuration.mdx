...
To reach the target through something other than `ssh`, set the program Herdr runs locally:
...
Herdr spawns `program` directly with this argument vector; no local shell is involved, so `{target}` and `{command}` each stay exactly one argument whatever they contain. `{target}` is the value passed to `--remote` and `{command}` is the remote command Herdr already builds, so the program must run that command on the target the way `ssh target "<command>"` does, with stdin and stdout connected: the long-running client bridge and the remote install both use them. The same launcher runs every remote command, including saved SSH machines, and `manage_ssh_config` no longer applies because Herdr generates no SSH config for it. A launcher that cannot be used is an error rather than a silent fall back to `ssh`.
