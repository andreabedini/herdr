...
`ssh` is not built in; it is the default value of `remote.command`, the local program that runs one remote command:
...
Herdr spawns `program` directly with this argument vector. No local shell is involved, so `{target}` (the value passed to `--remote`) and `{command}` (the remote command Herdr builds) each stay exactly one argument whatever they contain. `{ssh_options}` must stand alone as its own entry and expands to the OpenSSH options Herdr manages for that call: the generated config and control socket above, the non-interactive options saved machines need, and `-n` for commands that do not read stdin. Only an ssh command line wants it.

Replace both to reach the target through something else, for example a sandbox, container, or cluster CLI:
...
The program must run that command on the target the way `ssh target "<command>"` does, with stdin and stdout connected: the long-running client bridge and the remote install both use them. The same launcher runs every remote command, including saved SSH machines. Without `{ssh_options}` Herdr generates no SSH config for it, so `manage_ssh_config` has nothing to apply to. A launcher Herdr cannot use is an error, not a silent fall back.
