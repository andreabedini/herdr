...
`ssh` 并非内置，而是 `remote.command` 的默认值：它指定在本地运行一条远程命令的程序。
...
Herdr 会用这个参数向量直接启动 `program`。本地不经过任何 shell，因此无论内容如何，`{target}`（传给 `--remote` 的值）和 `{command}`（Herdr 构造的远程命令）都各自保持为一个参数。`{ssh_options}` 必须单独作为一个条目，它会展开为 Herdr 为该次调用管理的 OpenSSH 选项：上文生成的配置与 control socket、已保存机器需要的非交互选项，以及用于不读取 stdin 的命令的 `-n`。只有 ssh 命令行才需要它。

若要改用其他方式连接目标（例如沙箱、容器或集群 CLI），请同时替换这两项：
...
该程序必须像 `ssh target "<command>"` 那样在目标上运行该命令，并保持 stdin 与 stdout 连通：长时间运行的客户端桥接和远程安装都要用到它们。同一个启动器用于所有远程命令，包括已保存的 SSH 机器。没有 `{ssh_options}` 时 Herdr 不会为它生成 SSH 配置，因此 `manage_ssh_config` 也就无从应用。Herdr 无法使用的启动器配置会报错，而不会静默回退。
